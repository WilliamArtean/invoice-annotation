...
function test() {
    function loadImage(imageObj) {
      ...
      $.ajax({
        async: false,
        type : "GET",
        url : "/facture/"+urlParams.get('name'),
        contentType : "application/json",
        dataType : 'json',
        success: function(result){
          facture.id=result.id;
          facture.date=result.date;
          facture.numero=result.numero;
          facture.pages=result.pages;
          console.log("SUCCESS GET facture "+facture.id+" "+facture.numero+" "+facture.date+" "+facture.pages.length+" pages");
        },
        error : function(e) {
          console.log("ERROR GET facture: ", e);
        }
      });

      var imageObj = new Image();
      imageObj.src='img/'+facture.pages[0].content;
      ...
      button2.type = 'button';
      button2.innerHTML = 'Page 2';
      button2.className = 'btn-styled';

      button2.onclick = function(){
        imageObj.src='img/'+facture.pages[1].content;
      };

      var button3 = document.createElement('button');
      button3.type = 'button';
      button3.innerHTML = 'Page 3';
      button3.className = 'btn-styled';

      button3.onclick = function(){
        imageObj.src='img/'+facture.pages[3].content;
      };

      if(facture.pages.length>1){
        $('#topmenu').append(button1);
        $('#topmenu').append(button2);
      }
      else if(facture.pages.length>2){
        $('#topmenu').append(button1);
        $('#topmenu').append(button2);
    ...
    function saveAnnotations(){
      $.ajax({
        ...
        url : "/facture/"+urlParams.get('name')+"/save",
        contentType : "application/json",
        dataType : 'json',
        success: function(result){
        },
